...
async function initSupabaseData() {
    ...
    // **CORRECCIÓN:** Cargar TODOS los datos de Supabase (sin filtro de fecha) para poder ver historial de arqueos
    const a = await (window.db.obtenerArqueos ? window.db.obtenerArqueos() : window.db.obtenerArqueosPorFecha(fechaBase));
...
// Utilidades y formateo movidos a utils.js
// Utilidades movidas a utils.js
...
async function agregarMovimiento() {
    // **NUEVO:** Prevenir múltiples clics
    const btnPrincipal = document.querySelector('#modal-body .btn-primario');
    if (btnPrincipal && btnPrincipal.disabled) return;
    if (btnPrincipal) {
        btnPrincipal.disabled = true;
        btnPrincipal.dataset.originalText = btnPrincipal.textContent;
        btnPrincipal.textContent = 'Agregando...';
    }
    ...
    // **CORRECCIÓN:** Actualizar métricas después de agregar movimiento (ya se llama en renderizar)
        ...
        if (btnPrincipal) {
            ...
            btnPrincipal.textContent = btnPrincipal.dataset.originalText || 'Agregar Movimiento';
        }
    }
}
...
function renderizarVistaArqueoFinal(totales, todosLosEgresos = []) {
    ...
    // const totalAEntregar = totalEfectivoBruto; // Esta variable ya no se usa directamente en el resumen final como "a entregar"
    const totalAEntregarGs = totalEfectivoFinal - fondoFijo; // Esto sigue siendo el efectivo Gs a entregar después del fondo fijo
    ...
    const totalADeclarar = totalEgresosCaja + totalEfectivoBruto; // Total que el sistema calcula que debería haber
    const totalIngresosTiendaCalculado = (totalEfectivoBruto + totalEgresosCaja) - totalServiciosEfectivo - fondoFijo;

    // **NUEVO:** Cálculos para el arqueo manual
    const totalADeclararManual = totalEfectivoBruto + totalEgresosCaja; // Suma del efectivo contado + egresos
    const diferenciaArqueo = totalADeclararManual - totalADeclarar;
    const claseDiferencia = diferenciaArqueo < 0 ? 'negativo' : (diferenciaArqueo > 0 ? 'positivo' : '');
...
async function guardarArqueo() {
    ...
    // Poblar arqueo.resumen para el PDF
    arqueo.resumen = {
        totalEfectivo: arqueo.totalEfectivo,
        totalIngresos: totalIngresosParaGuardar,
        totalEgresosCaja: totalEgresosSumaParaResumen,
        totalADeclarar: parsearMoneda(document.getElementById('totalDeclararSistemaArqueo')?.textContent || '0'),
        totalIngresosTienda: totalIngresosTiendaCalculadoFinal,
        diferencia: parsearMoneda(document.getElementById('diferenciaArqueoFinal')?.textContent || '0')
    };
    ...
    guardarEnLocalStorage();
        ...
        if (btnGuardar) {
            ...
            btnGuardar.textContent = btnGuardar.dataset.originalText || '💾 Guardar Arqueo';
        }
    }
}
...
async function guardarGasto(event) {
    ...
    // **NUEVO:** Prevenir múltiples clics
    const btnSubmit = event && event.submitter ? event.submitter : document.activeElement;
    ...
    cargarResumenDiario();
        ...
        if (btnSubmit && btnSubmit.type === 'submit') {
            ...
            btnSubmit.textContent = btnSubmit.dataset.originalText || 'Guardar Movimiento';
        }
    }
}
...
async function guardarEgresoCaja(event) {
    ...
    cargarResumenDiario(); // **NUEVO:** Actualizar resumen en tiempo real
        ...
        if (btnSubmit && btnSubmit.type === 'submit') {
            btnSubmit.disabled = false;
            btnSubmit.textContent = btnSubmit.dataset.originalText || 'Guardar Egreso';
        }
    }
}
